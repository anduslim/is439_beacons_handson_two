var app = (function ()
{
    ...
    {
        document.getElementById("refresh2").textContent = "";
        document.getElementById("refresh3").textContent = "";
        for (var i = 0; i < beacons.length; ++i)
        {
            ...
            if (!mNearestBeacon)
            {
                ...
                if (!isSameBeacon(beacon, mNearestBeacon)) {
                    if (!isNearerThan(beacon, mNearestBeacon)) {
                        document.getElementById("refresh3").textContent = beacon.major + " : " + beacon.accuracy; 
                        mNearestBeacon = beacon;
                    }
        ...
        document.getElementById("refresh2").textContent += mNearestBeacon.major + " : " + mNearestBeacon.accuracy + " , " + beacon.major + " : " + beacon.accuracy;
    }
    ...
    function displayNearestBeacon()
    {
        ...
        // Update element.
        var beaconVal = "UUID: " + mNearestBeacon.uuid + "  Major: " + mNearestBeacon.major
                + "  Minor: " + mNearestBeacon.minor + "  Proximity: " + mNearestBeacon.proximity
                + "  Distance: " + mNearestBeacon.accuracy + "  RSSI: " + mNearestBeacon.rssi;
        document.getElementById("refresh").textContent = beaconVal;
    ...
    function displayRecentRegionEvent()
    {
        if (mAppInBackground)
        {
            ...
            cordova.plugins.notification.local.schedule({
                id: ++mNotificationId,
                title: title,
                text: "Distance: " + mNearestBeacon.accuracy});
        } else
        {
            displayRegionEvents();
        }
    ...
    function displayRegionEvents()
    {
        // Clear list.
        //$('#events').empty();
        document.getElementById("refresh1").textContent = "";
        document.getElementById("refresh4").textContent = "";
        ...
        for (var i = mRegionEvents.length - 1; i >= 0; --i)
        {
            ...
            /*
             ...
             //li.setAttribute("id", "element4"); //SET ID TO THE LI IF NEEDED
             ul.appendChild(li);
        ...
        document.getElementById("refresh1").textContent = result;
        document.getElementById("refresh4").textContent = mNearestBeacon.major + "  ,  " + mNearestBeacon.minor + "  , " + mNearestBeacon.accuracy;
        // If the list is empty display a help text.
        if (mRegionEvents.length <= 0)
        {
            document.getElementById("refresh1").textContent = "Waiting for region events, please move into or out of a beacon region.";
            /*
             var element = $(
             '<li>'
             + '<strong>'
